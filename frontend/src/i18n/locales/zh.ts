  ...
  admin: {
    ...
    channelMonitor: {
      ...
      form: {
        ...
        endpointHint: '可填 origin（如 https://api.example.com）或带路径前缀的 URL（如 https://gateway.com/model-farm/vllm_api）。不要包含 query / fragment。',
        endpointJoinIntro: '检测时会在上游地址后拼接以下路径：',
        endpointJoinPreviewLabel: '实际请求 URL：',
        endpointJoinPreviewPlaceholder: '（请先填写上游地址）',
        apiKey: 'API Key',
        apiKeyPlaceholder: '请输入 API Key',
